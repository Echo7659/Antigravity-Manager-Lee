...
impl CanonicalUsage {
    ...
    pub fn from_gemini(raw: &Value) -> Self {
        ...
        let output = if has_new_format {
            ...
            // 旧版 Gemini 的思考计数可能独立或已合并，使用上游总量消歧。
            crate::proxy::mappers::usage::gemini_output_tokens(usage).unwrap_or(raw_output)
        };
...
mod tests {
    ...
    #[test]
    fn compat_canonical_usage_preserves_legacy_reasoning_and_all_protocol_totals() {
        for (answer, thinking, total, expected) in
            [(20, 30, 150, 50), (50, 30, 150, 50), (0, 7, 107, 7)]
        {
            let usage = CanonicalUsage::from_gemini(&json!({
                "promptTokenCount": 100, "candidatesTokenCount": answer,
                "thoughtsTokenCount": thinking, "totalTokenCount": total
            }));
            assert_eq!(usage.output_tokens, expected);
            assert_eq!(usage.to_openai_chat_usage()["completion_tokens"], expected);
            assert_eq!(usage.to_openai_responses_usage()["output_tokens"], expected);
            assert_eq!(
                usage.to_claude_usage(false, 200_000)["output_tokens"],
                expected
            );
        }
    }
