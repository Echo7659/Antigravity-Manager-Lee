...
#[cfg(test)]
mod lee_compat_tests {
    use super::UpstreamClassification;

    #[test]
    fn compat_only_rate_limits_trigger_account_cooldown() {
        for status in [400, 404, 500, 502, 503, 504] {
            assert!(
                !UpstreamClassification::classify(status, "upstream error", None)
                    .should_lock_account()
            );
        }
        for status in [429, 529] {
            assert!(
                UpstreamClassification::classify(status, "RESOURCE_EXHAUSTED", Some("2"))
                    .should_lock_account()
            );
        }
        assert!(
            !UpstreamClassification::classify(429, "All accounts limited", None)
                .should_lock_account()
        );
    }
}
