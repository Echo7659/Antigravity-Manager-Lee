            ...
            .and_then(|m| m.as_array())
        {
            for model in models {
                if let (Some(name), Some(pct)) = (
                    ...
                    model_quotas
                        .entry(standard_id)
                        .and_modify(|old| *old = (*old).min(pct as i32))
        ...
        tokens_snapshot.sort_by(|a, b| {
            ...
            compare_accounts(
                AccountRank {
                    ...
                    quota: crate::proxy::quota_policy::model_percentage(
                        &a.exact_model_quotas,
                        &a.model_quotas,
                        target_model,
                        &normalized_target,
                    )
                    ...
                    if let Some(wait_sec) = min_wait {
                        if wait_sec <= 2 {
                            ...
                            if let Some(t) = retry_token {
                                ...
                                return Err(
                                    "All accounts remain limited after retry delay.".to_string()
                                );
                            }
                        } else {
